...
std::ifstream in("C:\\Users\\coovs\\Desktop\\Лабы Метра\\Lab3\\in.txt");
...
{
    int save = next;
    if (terms[next].type != INPUTb && terms[next].type != PRINT)
        return;
    ++next;
